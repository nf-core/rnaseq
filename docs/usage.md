...
* appended `'bam':''` to default value of `publish_files` so that the bam files generated by the process will also be saved in the top-level results directory for the module. Note: `'out':'log'` means any file/directory ending in `out` will now be saved in a separate directory called `my_star_directory/log/`.

 as well as the file types generated by that process 
