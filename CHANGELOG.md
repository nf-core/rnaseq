...
- [PR #1915](https://github.com/nf-core/rnaseq/pull/1915) - Add `manifest.diagram` to `nextflow.config`, pointing at the metro map; wire 39 of 54 metro map stations to their Nextflow fully-qualified process names via `%%metro process_scope:`/`%%metro process:` directives so `nf-metro serve`/`check-mapping` can drive live-progress mode, fix several topology inaccuracies in `assets/metro_map.mmd`, and re-render with nf-metro >=2.0.0, which now natively supports light/dark mode
