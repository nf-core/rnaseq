...
include { SALMON_SUMMARIZEDEXPERIMENT as SALMON_SE_GENE
          SALMON_SUMMARIZEDEXPERIMENT as SALMON_SE_GENE_LS
          SALMON_SUMMARIZEDEXPERIMENT as SALMON_SE_GENE_S
...
workflow QUANTIFY_SALMON {
    ...
    SALMON_SE_GENE_LS ( 
        SALMON_MERGE_COUNTS.out.ls_counts_gene,
        SALMON_MERGE_COUNTS.out.ls_tpm_gene,
        SALMON_TX2GENE.out.collect(),
    )
    SALMON_SE_GENE_S ( 
        SALMON_MERGE_COUNTS.out.s_counts_gene,
        SALMON_MERGE_COUNTS.out.s_tpm_gene,
        SALMON_TX2GENE.out.collect(),
    )
    ...
    tpm_gene_ls                  = SALMON_TXIMPORT.out.ls_tpm_gene           // channel: [ val(meta), counts ]
    counts_gene_ls               = SALMON_TXIMPORT.out.ls_counts_gene        // channel: [ val(meta), counts ]
    tpm_gene_s                   = SALMON_TXIMPORT.out.s_tpm_gene            // channel: [ val(meta), counts ]
    counts_gene_s                = SALMON_TXIMPORT.out.s_counts_gene         // channel: [ val(meta), counts ]
    ...
    merged_counts_gene_ls        = SALMON_MERGE_COUNTS.out.ls_counts_gene    //    path: *.gene_counts.tsv
    merged_tpm_gene_s            = SALMON_MERGE_COUNTS.out.ls_tpm_gene       //    path: *.gene_tpm.tsv
    merged_counts_gene_ls        = SALMON_MERGE_COUNTS.out.s_counts_gene     //    path: *.gene_counts.tsv
    merged_tpm_gene_s            = SALMON_MERGE_COUNTS.out.s_tpm_gene        //    path: *.gene_tpm.tsv
    merged_gene_rds              = SALMON_SE_GENE.out.rds                    //    path: *.rds
    merged_gene_rds_ls           = SALMON_SE_GENE_LS.out.rds                 //    path: *.rds
    merged_gene_rds_s            = SALMON_SE_GENE_S.out.rds                  //    path: *.rds
