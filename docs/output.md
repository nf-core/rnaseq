...
The [tximport](https://bioconductor.org/packages/release/bioc/html/tximport.html) package is used in this pipeline to summarise the results generated by Salmon into matrices for use with downstream gene-level analysis packages. We use tximport with different options to summarize count and TPM quantifications at the gene-level i.e. `countsFromAbundance=NULL` (`salmon.merged.gene_counts.tsv`), `countsFromAbundance="ScaledTPM"` (`salmon.merged.gene_counts_scaled.tsv`) and `countsFromAbundance="lengthScaledTPM"` (`salmon.merged.gene_counts_length_scaled.tsv`) as well as at the transcript-level i.e. `salmon.merged.transcript_counts.tsv`.

Please see [#499](https://github.com/nf-core/rnaseq/issues/499) for discussion and links regarding which counts are suitable for differential analysis.
