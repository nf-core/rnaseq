...
one shows no measurable change despite being closed with no linked PR.
...
## Not fixed despite being closed - reopen or clarify upstream

| # | Original finding | Empirical status (2026-08-27) |
|---|---|---|
| [#31](https://github.com/scverse/rustar-aligner/issues/31) | NH tail extends deeper than STAR's (originally 20 vs 6-7); looked like `--outFilterMultimapScoreRange` wasn't applied | **No measurable change.** `--outFilterMultimapScoreRange` is now advertised in `--help` with the correct default (`1`, matching STAR), but the empirical NH distribution on WT_REP2 is essentially unchanged: rustar still reaches NH:20 with 18/90680 primary reads at NH>7, while STAR's max in the same file is NH:7. The issue was closed with **no linked PR** (`Development: No branches or pull requests` on the issue page) - nothing in the codebase appears to have addressed it. Recommend reopening or filing a fresh issue with this re-run's numbers attached, since the original repro no longer applies verbatim (rustar has since moved from 0.1.0 to 0.2.0) but the underlying behaviour is identical. |
| [#48](https://github.com/scverse/rustar-aligner/issues/48) | `Log.final.out` folds all unmapped reads into `too short`; `other` bucket always 0 | **No measurable change.** WT_REP2: rustar reports `too short: 4189 (8.45%)`, `other: 0 (0.00%)`. STAR on the identical input reports `too short: 1540 (3.11%)`, `other: 2656 (5.36%)` - a real categorical split that rustar still collapses entirely into one bucket. Total unmapped-non-mismatch count is close (4189 vs 4196) so this isn't a counting bug, it's a mis-categorisation: whatever STAR's logic is for `other` (typically: too many mismatches after trimming, or reads that fail the pair-concordance requirement rather than the length filter), rustar isn't replicating it. Worth a fresh, more targeted upstream issue since #48's original repro predates 0.2.0. |
