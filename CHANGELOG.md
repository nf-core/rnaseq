...
- [PR #XXXX](https://github.com/nf-core/rnaseq/pull/XXXX) - Fix per-sample MultiQC stalling in `--skip_quantification_merge` mode via structural join-based bundle assembly: each per-sample MultiQC report fires as soon as that sample's contributing processes finish, not after the slowest sample's tasks end. Versions are collected via `.first()` taps on named process outputs so the versions YAML closes bounded by "one sample's worth of processing per process" rather than the workflow-global topic close ([#1797](https://github.com/nf-core/rnaseq/issues/1797))
