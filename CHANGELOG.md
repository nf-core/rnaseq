...
- [PR #1923](https://github.com/nf-core/rnaseq/pull/1923) - Add `--extra_salmon_index_args`, letting users pass `--keepDuplicates` (or other Salmon index flags) without changing the pipeline default of collapsing exact-sequence-duplicate transcripts; also fixes `quant_tximport_summarizedexperiment` failing on any transcript absent from the reference GTF ([#1259](https://github.com/nf-core/rnaseq/issues/1259))
