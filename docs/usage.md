...
To further assist in reproducbility, you can use share and re-use [parameter files](#using-a-params-file) to repeat pipeline runs with the same settings without having to write out a command with every single parameter.
...
This section describes how to configure the pipeline for specific use cases or library types.
...
- Use `--featurecounts_feature_type 'CDS' --featurecounts_group_type 'product'` to identify the correct biotype identifier, although this may depend on the annotation format you are using and you may have to try a few combinations to find what works.

A quick way to check the entries for your annotation file is to use `grep` to search for the term you would like to use for `--featurecounts_feature_type` e.g. `grep -P '\ttranscript\t' <GFF_FILE> | head`. The 3rd field should contain the text you are searching for and the last column should ideally contain the text defined by `--featurecounts_group_type`.

A simplified config to run nf-core/rnaseq for prokaryotic organisms is provided and can be called via `-profile prokaryotic`. It sets the `--skip_biotype_qc`, `--skip_rseqc`, and `--featurecounts_feature_type 'CDS'` parameters, as well as `--bam_csi_index` (since prokaryotic genomes are typically very small).
...
#### Custom STAR parameters for 3' assays
...
#### Custom Salmon arguments for 3' assays
...
The pipeline supports GPU-accelerated alignment with NVIDIA Parabricks and Sentieon tools. These require appropriate hardware and licenses:

- Parabricks: Use `--use_parabricks_star` to enable GPU-accelerated STAR alignment
- Sentieon: Use `--use_sentieon_star` to enable GPU-accelerated STAR alignment

For Parabricks, you may also need to specify GPU-specific container options with `--gpu_container_options`.

## Reference: Pipeline stages

This section provides detailed information about each stage of the pipeline and its configuration options.
...
The pipeline config contains references to iGenomes. If running with docker or singularity, the images containing the reference will be pulled automatically. For more information about the iGenomes references, please see the [reference genomes page](https://nf-co.re/usage/reference_genomes).
