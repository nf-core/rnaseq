...
- Every subworkflow declares its record type(s) in a sibling `types.nf`, local and nf-core-owned alike, and `include`s them into `main.nf` - the same convention everywhere, no exceptions. A type used by more than one component (a shared samtools-stats shape, `Rseqc`, `QuantMerged`, ...) is declared once, in whichever component the others already depend on, and `include`d from there rather than copied.
- Nf-core-owned components under `subworkflows/nf-core/` are, for now, edited in place without going through `nf-core subworkflows patch`/`.diff` tracking. That tooling can't capture full content for a brand-new file in its generated diff (a bare "was created" note), so a future `nf-core subworkflows update` would silently delete a component's `types.nf` while leaving behind the `include` line that references it - a known, tracked nf-core/tools limitation (nf-core/tools#3157), not something to route around by restructuring this pipeline's own code. Until that's fixed upstream, `nf-core pipelines lint`/`nf-core subworkflows lint` will flag these components as diverged from their patch; that's expected, not a bug in the pipeline.
