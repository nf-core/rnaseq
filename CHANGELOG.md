...
- [PR #1876](https://github.com/nf-core/rnaseq/pull/1876) - Wire 39 of 54 metro map stations to their Nextflow fully-qualified process names via `%%metro process_scope:`/`%%metro process:` directives so `nf-metro serve`/`check-mapping` can drive live-progress mode, fix several topology inaccuracies in `assets/metro_map.mmd`, re-render with nf-metro 1.1.0, and switch the README map from the standalone `light` theme to the paired `nfcore` theme with a light/dark logo, so it now follows the viewer's light/dark setting instead of staying fixed
