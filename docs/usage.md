...
When STAR has been chosen as the aligner then the pipeline sets the minimal lenght of an alignment to 15. This is to avoid some strange very short mappings that can come from the soft-clipping in STAR. If you which to change this value then you can modify it with the following flag:

`--params.minAlgined`
Sensitivity can be increased by making it higher, or decreased by dropping it. STARS own default is 0. 
## Reference Genomes
