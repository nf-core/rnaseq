...
Once you have the indices from a workflow run you should save them somewhere central and reuse them in subsequent runs. The simplest way to do this is with the `saved_reference.yaml` params file that is written to `<outdir>/genome/` when using `--save_reference`:

```bash
nextflow run nf-core/rnaseq \
    -params-file /path/to/previous_results/genome/saved_reference.yaml \
    --input new_samplesheet.csv \
    --outdir /path/to/new_results
```

This params file contains absolute paths to all saved reference files and indices. Any parameters specified on the command line (such as `--outdir` above) override values from the params file.

Alternatively, you can specify individual reference paths using custom config files or command line parameters such as `--star_index '/path/to/STAR/index/'`.

> [!NOTE]
> The params file only contains references that were generated by the pipeline. If you provided pre-built indices (e.g., `--star_index /my/index`), you will need to specify those again. Input files like `--bbsplit_fasta_list` and `--ribo_database_manifest` are also not included and must be re-specified if needed.
