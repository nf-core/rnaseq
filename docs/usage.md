...
:::note
By selecting `star_rsem`, the following star aligner settings are automatically set. These are based on the ENCODE3 STAR settings, and are equivalent to what `rsem-calculate-exprression` sets when using the `--star` option:
...
To adjust these, you cannot use [`--extra_star_align_args`](#custom-star-parameters) and rather need to override the process with name `STAR_ALIGN` in a custom config file.
:::
