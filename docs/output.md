...
* `pipeline_info/`
    ...
    * Reports generated by the pipeline: `pipeline_report.html`, `pipeline_report.txt` and `software_versions.tsv`. The `pipeline_report*` files will only be present if the `--email` parameter is used when running the pipeline.
