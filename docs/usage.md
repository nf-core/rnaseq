...
**NOTE**: With `star_rsem`, STAR runs as a standalone alignment step (the same `STAR_ALIGN` process used by `star_salmon`) and the resulting transcriptome BAM is quantified by RSEM in `--alignments` mode, so STAR is never invoked internally by RSEM. As a result, [`--extra_star_align_args`](#custom-star-parameters) applies to `star_rsem` exactly as it does to `star_salmon`: the flags you supply are merged with the STAR settings above, with your values overriding the defaults on a clash.
:::
...
By default, Salmon collapses transcripts with identical sequence during indexing: only one representative of each set of exact-sequence duplicates is kept in the index, and the rest are dropped from `quant.sf` entirely (see the [Salmon docs](https://github.com/COMBINE-lab/salmon/issues/214#issuecomment-381580235)). This is deliberate, since duplicate sequences are statistically indistinguishable and Salmon cannot assign reads between them, but it means a transcript you supplied can be silently absent from the results with no error. Every duplicate cluster, dropped or not, is recorded in `duplicate_clusters.tsv` inside the Salmon index directory (`--save_reference`). If you'd rather retain all transcripts, pass Salmon's `--keepDuplicates` flag via `--extra_salmon_index_args`:

```bash
--extra_salmon_index_args "--keepDuplicates"
```

Two additional parameters `--extra_star_align_args` and `--extra_salmon_quant_args` were added in v3.10 of the pipeline that allow you to append any custom parameters to the STAR align and Salmon quant commands, respectively. Note, the `--seqBias` and `--gcBias` are not provided to Salmon quant by default so you can provide these via `--extra_salmon_quant_args '--seqBias --gcBias'` if required. You can now also supply additional arguments to Kallisto via `--extra_kallisto_quant_args`, and to the Salmon index command via `--extra_salmon_index_args` (as above with `--keepDuplicates`); as with the other `--extra_*_args` parameters, a value you supply here takes priority over the pipeline's own `--gencode`/`-k` flags if it duplicates one of them.
...
The pipeline provides the option to scan reads for contamination from other species using either [Sylph](https://sylph-docs.github.io/) or [Kraken2](https://ccb.jhu.edu/software/kraken2/), with the possibility of applying corrections from [Bracken](https://ccb.jhu.edu/software/bracken/). The `--contaminant_screening_input` parameter controls which reads are screened:

| Value                | Reads screened                                                   | Notes                                                                                                                                            |
| -------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `unmapped` (default) | Aligner-unmapped reads                                           | Requires `--aligner star_salmon`, `star_rsem`, or `hisat2`. Focuses on reads rejected by STAR/HISAT2.                                            |
| `trimmed`            | Post-BBSplit / post-rRNA-removal reads entering alignment        | Useful to inspect what survives all pre-alignment filters.                                                                                       |
| `trim_only`          | Post-adapter-trimming reads, **before** BBSplit and rRNA removal | Recommended when BBSplit uses a species-limited index (e.g. mouse-only) that could discard expected contaminants before they reach the screener. |
| `raw`                | Pre-trimming reads (adapter sequences still present)             | Baseline contamination check; note that adapter k-mers may inflate some Kraken2 hits.                                                            |
