...
def parse_args(args=None):
    ...
    parser.add_argument(
        ...
        default="",
        help="Comma-separated list of ENA metadata fields to fetch. (default: {}).".format(
            ",".join(ENA_METADATA_FIELDS)
        ),
    )
...
def validate_csv_param(param, valid_vals, param_desc):
    ...
    if param:
        ...
        if len(intersect) == len(user_vals):
            ...
            print(
                "ERROR: Please provide a valid value for {}!\nProvided values = {}\nAccepted values = {}".format(
                    param_desc, param, ",".join(valid_vals)
                )
            )
            sys.exit(1)
    return valid_list
            ...
            if match:
                ...
                if prefix in PREFIX_LIST:
                    if not db_id in seen_ids:
                        ...
                        if not ids:
                            print(
                                "ERROR: No matches found for database id {}!\nLine: '{}'".format(
                                    db_id, line.strip()
                                )
                            )
                            sys.exit(1)
