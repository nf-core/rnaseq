...
- `bowtie2_rrna/`
  ...
  - `*.fastq.gz`: If `--save_non_ribo_reads` is specified, FastQ files containing non-rRNA reads will be placed in this directory.
  - `index/`: If `--save_reference` is specified and no `--bowtie2_rrna_index` is given, the Bowtie2 index built from the rRNA databases.
- `seqkit/`: When the Bowtie2 rRNA index is built by the pipeline, the prepared rRNA databases it is built from.
  - `*_prefixed.fasta`: rRNA database with the database name prefixed to each sequence ID.
  - `*_dna_converted.fasta`: The prefixed database with U converted to T.
...
- `<ALIGNER>/`
  ...
  - `<SAMPLE>.transcriptome.sorted.bam`, `<SAMPLE>.transcriptome.sorted.bam.bai`: If `--save_umi_intermeds` or `--save_align_intermeds` is specified with `--aligner star_salmon` or `--aligner star_rsem`, the coordinate sorted transcriptome BAM file (and its index) that goes into transcriptome-level UMI deduplication.
  - `<SAMPLE>.umi_dedup.transcriptome.sorted.bam`, `<SAMPLE>.umi_dedup.transcriptome.sorted.bam.bai`: If `--save_umi_intermeds` or `--save_align_intermeds` is specified with `--aligner star_salmon` or `--aligner star_rsem`, the UMI deduplicated, coordinate sorted transcriptome BAM file and its index.
  - `<SAMPLE>.umi_dedup.transcriptome.bam`, `<SAMPLE>.umi_dedup.transcriptome.filtered.bam`: If `--save_umi_intermeds` or `--save_align_intermeds` is specified with `--aligner star_salmon` or `--aligner star_rsem`, the name sorted deduplicated transcriptome BAM file and the same file after UMI-tools `prepare-for-rsem`, which is the input to quantification.
- `<ALIGNER>/samtools_stats/`
  - SAMtools `.stats`, `.flagstat` and `.idxstats` files for `<SAMPLE>.umi_dedup.sorted.bam` and, with `--aligner star_salmon` or `--aligner star_rsem`, for `<SAMPLE>.umi_dedup.transcriptome.sorted.bam`. The same files for `<SAMPLE>.transcriptome.sorted.bam` are published when `--save_umi_intermeds` or `--save_align_intermeds` is specified.
- `<ALIGNER>/umitools/prepare_for_quantification_log/`: UMI-tools `prepare-for-rsem` logs (`--aligner star_salmon` or `--aligner star_rsem`).
- `<ALIGNER>/<umitools|umicollapse>/genomic_dedup_log/`, `<ALIGNER>/<umitools|umicollapse>/transcriptomic_dedup_log/`: Deduplication logs for the genome and (with `--aligner star_salmon` or `--aligner star_rsem`) transcriptome BAM files.
