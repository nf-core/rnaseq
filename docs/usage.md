...
By default, the pipeline uses [STAR](https://github.com/alexdobin/STAR) to align the raw FastQ reads to the reference genome. STAR is fast and common, but requires a lot of memory to run, typically around 38GB for the Human GRCh37 reference genome. After STAR, users can also use the RSEM method for quantification. If this is not desired, `--skip_rsem` turns off RSEM after STAR.
