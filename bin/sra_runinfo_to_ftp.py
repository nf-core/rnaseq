            ...
            if library == "SINGLE":
                ...
                if fastq_files:
                    ...
                    if len(fq_files) == 2:
                        ...
                        print(
                            "Invalid number of FastQ files ({}) found for paired-end database id:'{}'!.".format(
                                len(fq_files), run_id
                            )
                        )
                else:
...
def sra_runinfo_to_ftp(files_in, file_out):
    ...
    if samplesheet_dict:
        ...
        with open(file_out, "w") as fout:
            header = ["id"] + list(
                samplesheet_dict[list(samplesheet_dict.keys())[0]][0].keys()
            )
            ...
            for db_id in sorted(samplesheet_dict.keys()):
                for idx, val in enumerate(samplesheet_dict[db_id]):
                    fout.write(
                        "\t".join(
                            ["{}_T{}".format(db_id, idx + 1)]
                            + [val[x] for x in header[1:]]
                        )
                        + "\n"
                    )
