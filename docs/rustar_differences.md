...
confirmed against a `:dev` build pulled minutes old at the tip of `main`.
Both closures turn out to be premature: the PR each issue cites (#77, #141)
fixes a different, adjacent bug in the same subsystem, not the one
reported. See `rustar_reverification_2026-08.md` for the read-the-actual-diff
evidence. The tables below are left as the historical record of what the
...
| [#31](https://github.com/scverse/rustar-aligner/issues/31) | medium | Multi-mapper NH cap extends to 20 vs STAR's 7; ~17% more secondaries on identical input. Possibly missing an `--outFilterMultimapScoreRange`-equivalent threshold. | 🔴 Closed citing PR #77, but that PR fixes `too_many_loci` accounting (PR #54), not the NH-depth cap - **confirmed still broken** against a `:dev` build minutes old | [`rustar_bam_comparison.md`](rustar_bam_comparison.md), [`rustar_reverification_2026-08.md`](rustar_reverification_2026-08.md) |
...
| [#48](https://github.com/scverse/rustar-aligner/issues/48) | medium | `Log.final.out` folds all unmapped reads into `too short`; `other` bucket always 0. | 🔴 Closed citing PR #141, but that PR fixes `too_many_loci`-vs-`too_short` mislabeling, not the `too_short`-vs-`other` split - **confirmed still broken** against a `:dev` build minutes old | [`rustar_reverification_2026-08.md`](rustar_reverification_2026-08.md) |
